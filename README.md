...
| `Codex:ModelDiscoveryTimeoutSeconds` | `30` | Maximum time for the isolated runner to return the authenticated account's model catalog; expiry makes the request temporarily unavailable |
...
| MCP gateway core | Creates run-scoped sessions with opaque tokens, rejects tool calls outside the project/API-key grant, selects an explicit transport factory, and removes completed or expired sessions. | Registration/options are in [`McpGatewayInstaller.cs`](src/CodexGateway.McpGateway/Composition/McpGatewayInstaller.cs) and [`GatewayMcpOptions.cs`](src/CodexGateway.McpGateway/Configuration/Models/GatewayMcpOptions.cs). Change session/token behavior in [`GatewayMcpSessionManager.cs`](src/CodexGateway.McpGateway/Sessions/GatewayMcpSessionManager.cs), tool-call enforcement in [`GatewayMcpToolCallAuthorizer.cs`](src/CodexGateway.McpGateway/Sessions/GatewayMcpToolCallAuthorizer.cs), transport contracts in [`Transport`](src/CodexGateway.McpGateway/Transport/), and the internal runner-facing route in [`GatewayMcpEndpointExtensions.cs`](src/CodexGateway.App/Mcp/Endpoints/GatewayMcpEndpointExtensions.cs). |
| HTTP MCP gateway transport | Forwards runner MCP requests to remote HTTP MCP servers and applies environment-backed upstream authentication without exposing it to the runner. | Registration is in [`HttpMcpGatewayInstaller.cs`](src/CodexGateway.McpGateway.Http/Composition/HttpMcpGatewayInstaller.cs). Change credential resolution in [`HttpMcpUpstreamFactory.cs`](src/CodexGateway.McpGateway.Http/Transport/HttpMcpUpstreamFactory.cs) and request/response forwarding in [`HttpGatewayMcpUpstream.cs`](src/CodexGateway.McpGateway.Http/Transport/HttpGatewayMcpUpstream.cs). |
| STDIO MCP gateway transport | Starts one local STDIO MCP process per run-scoped session, relays JSON-RPC through stdin/stdout, and terminates the process with the session. | Registration is in [`StdioMcpGatewayInstaller.cs`](src/CodexGateway.McpGateway.Stdio/Composition/StdioMcpGatewayInstaller.cs). Change environment resolution in [`StdioMcpUpstreamFactory.cs`](src/CodexGateway.McpGateway.Stdio/Transport/StdioMcpUpstreamFactory.cs) and process/protocol behavior in [`LocalStdioGatewayMcpUpstream.cs`](src/CodexGateway.McpGateway.Stdio/Transport/LocalStdioGatewayMcpUpstream.cs). |
...
| Configuration | Defines validated option shapes and their deployment values. The option class explains meaning; `appsettings.json`, environment variables, Aspire, and Compose provide values. | Gateway/run/file/artifact settings are in [`Logic/Configuration/Models`](src/CodexGateway.Logic/Configuration/Models/); MCP settings in [`GatewayMcpOptions.cs`](src/CodexGateway.McpGateway/Configuration/Models/GatewayMcpOptions.cs); admin settings in [`AdminUiOptions.cs`](src/CodexGateway.App/Admin/Authentication/Models/AdminUiOptions.cs). Defaults are in [`appsettings.json`](src/CodexGateway.App/appsettings.json), Aspire wiring in [`AppHost/Program.cs`](src/CodexGateway.AppHost/Program.cs), and container deployment in [`compose.yaml`](compose.yaml). Update the [configuration table](#configuration) when adding or renaming a public setting. |
