...
The UI can globally disable a catalog server and choose where it executes. **Gateway** is the default for new servers: the runner is configured with a temporary internal MCP URL and scoped bearer token, while the gateway adds the HTTP API key or starts the configured local STDIO command. The internal endpoint fixes the configured upstream server and rejects missing, expired, or incorrect tokens. HTTP gateway servers currently support static bearer API keys only; browser/OAuth authorization is not supported.
...
**Runner** is an advanced compatibility mode for binaries that must run inside the selected runner image. Their executable paths must exist in that image, and they share the runner's workspace, internet access, and Codex login state. Runner-hosted servers cannot use gateway-managed API-key bindings. The default image intentionally contains only Codex, Bubblewrap, Node/npm, certificates, and Ubuntu base utilities; build a derived runner image for any such binary.

Remote bearer tokens and STDIO secrets are referenced by environment-variable name and are never placed in process arguments. Gateway-hosted values never enter the runner container; runner-hosted values are excluded from model-generated shell environments.
