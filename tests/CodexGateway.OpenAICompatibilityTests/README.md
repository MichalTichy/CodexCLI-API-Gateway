...
1. `Infrastructure/CompatibilityGatewayFactory.cs` starts an isolated gateway backed by a disposable PostgreSQL database, and `WireCaptureHandler.cs` records the SDK's sanitized HTTP exchange.
...
This project pins `OpenAI` `2.13.0` and exercises its real Chat Completions client against the Gateway's production Marten persistence path. The pin is the Gateway compatibility-suite pin, selected as the current stable NuGet release on 2026-08-17.
